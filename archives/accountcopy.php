...
if (@$_GET['q'] == 1) {
    ...
    $result = mysqli_query($con, "SELECT * FROM unit1ez WHERE status = 'enabled' ORDER BY date DESC") or die('Error');
    ...
    <td style="vertical-align:middle">Time Limit</td>
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        if ($rowcount == 0) {
            echo '<tr>
                    <td style="vertical-align:middle;  style="width: 100%;"">' . $c++ . '</td>
                    <td style="vertical-align:middle;">' . $title . '</td>
                    <td style="vertical-align:middle">' . $total . '</td>
                    <td style="vertical-align:middle">' . $time . '</td>    
                    <td style="vertical-align:middle"><b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="color:white; background-color:#148110;width: 100%; padding: 5px;border-bottom-left-radius: 5px;border-bottom-right-radius: 5px;border-top-left-radius: 5px;border-top-right-radius: 5px; font-size: 15px;">Start</td>
                </tr>';
        } else {
            ...
            if ($remaining > 0 && $qstatus == "enabled" && $status == "ongoing") {
                ...
                <b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="color:white;background-color:#f1c40f;padding: 5px; border-bottom-left-radius: 5px;
                ...
                <td style="vertical-align:middle">' . $time . '</td>  
                <td style="vertical-align:middle">
                <b><a href="account.php?q=result&eid=' . $eid . '" class="btn" style="color:white;background-color:#1C7947;padding: 5px; border-bottom-left-radius: 5px;
                 border-bottom-right-radius: 5px;border-top-left-radius: 5px; border-top-right-radius: 5px;"></span>&nbsp;&nbsp;View&nbsp;&nbsp;</a></b></td></tr>';
            }
        }
    }
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_GET['endquiz'])== 'end') {
    ...
    $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[username]'") or die('Error156');
            ...
            header('location:accountcopy.php?q=result&eid=' . $_GET['eid']);
}
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_GET['start']) && $_GET['start'] == "start" && (!isset($_SESSION['6e447159425d2d']))) {
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        $q = mysqli_query($con, "SELECT * FROM unit1ez WHERE eid='$_GET[eid]' ") or die('Error197');
        while ($row = mysqli_fetch_array($q)) {
            $ttimel  = $row['time'];
            $qstatus = $row['status'];
        }
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            header('location:accountcopy.php?q=quiz&step=2&eid=' . $_GET['eid'] . '&n=' . $_GET['n'] . '&t=' . $_GET['t']);
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            var seconds = ' . $remaining . ' ;
                ...
                function end(){{
                    window.location ="accountcopy.php?q=quiz&step=2&eid=' . $_GET["eid"] . '&n=' . $_GET["n"] . '&t=' . isset($_GET["total"]) . '&endquiz=end";
                }
                }
            ...
            $sn    = @$_GET['n'];
            $total = @$_GET['t'];
            $q     = mysqli_query($con, "SELECT * FROM questions2 WHERE eid='$eid' AND sn='$sn' ");
            ...
            echo '<form id="qform" action="update2.php?q=quiz&step=2&eid=' . $eid . '&n=' . $sn . '&t=' . $total . '&qid=' . $qid . '" method="POST"  class="form-horizontal"><br />';
            ...
            if (strlen($ans) > 0) {
                echo "<font style=\"color:green;font-size:12px;font-weight:bold\">Selected answer: </font><font style=\"color:#565252;font-size:12px;\">" . $ans . "</font>&nbsp;&nbsp;<a href=update2.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total&qid=$qid&delanswer=delanswer><span class=\"glyphicon glyphicon-remove\" style=\"font-size:12px;color:darkred\"></span></a><br /><br />";
            }
            ...
            $q = mysqli_query($con, "SELECT * FROM questions2 WHERE eid='$_GET[eid]'") or die("Error222");
            $i = 1;
            while ($row = mysqli_fetch_array($q)) {
                $ques[$row['qid']] = $i;
                $i++;
            ...
            for ($i = 1; $i <= $total; $i++) {
                echo '<a href="accountcopy.php?q=quiz&step=2&eid=' . $eid . '&n=' . $i . '&t=' . $total . '"  style="margin:5px;padding:5px;background-color:';
                if ($quesans[$i]) {
                    echo "#9b59b6";
                } else {
                    echo "#c0392b";
...
if (@$_GET['q'] == 'result' && @$_GET['eid']) {
    $eid = @$_GET['eid'];
    $q = mysqli_query($con, "SELECT * FROM unit1ez WHERE eid='$eid' ") or die('Error157');
    while ($row = mysqli_fetch_array($q)) {
        $total = $row['total'];
    }
    ...
    while ($row = mysqli_fetch_array($q)) {
        ...
        $q = mysqli_query($con, "SELECT * FROM questions2 WHERE eid='$_GET[eid]'") or die('Error197');
        while ($row = mysqli_fetch_array($q)) {
            $question = $row['qns'];
            $qid      = $row['qid'];
...
if (@$_GET['q'] == 2) {
   ...
   echo '<div class="panel title">
    ...
    while ($row = mysqli_fetch_array($q)) {
        ...
        $w   = $row['wrong'];
        $r   = $row['correct'];
        $q23 = mysqli_query($con, "SELECT * FROM unit1ez WHERE  eid='$eid' ") or die('Error208');
        ...
        <td style="vertical-align:middle"><a href="accountcopy.php?q=result&eid=' . $eid . '" class="btn" style="color:white;background-color: #007939c2;padding: 5px; border-bottom-left-radius: 5px;
...
<tr><td style="vertical-align:middle"><b>Rank</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle; width:100%;"><b>Score</b></td></tr>';
    ...
    if($show == 1 && $total==1){
        ...
        $i = 1;
        while($i<=$total){
            echo '<td style="vertical-align:middle;text-align:center"><a style="font-size:14px;font-family:typo;font-weight:bold" href="accountcopy.php?q=3&show='.$i.'">&nbsp;'.$i.'&nbsp;</a></td>';
            $i++;
        }
        echo '<td style="vertical-align:middle;text-align:center"><a style="color:white;font-size:16px;font-family:calibri;border-radius:4px; background-color: #125C13;" href="accountcopy.php?q=3&show='.($show+1).'">&nbsp;<i class="fa fa-forward fa fw"></i>&nbsp;</a></td>';
    }
    else if($show != 1 && $show==$total){                                                                                                                                                                       /* Arrow next*/
        echo '<td style="vertical-align:middle;text-align:center"><a style="color:white;font-size:16px;font-family:calibri;border-radius:4px; background-color: #125C13;" href="accountcopy.php?q=3&show='.($show-1).'">&nbsp;<i class="fa fa-backward fa fw"></i>&nbsp;</a></td>';
        ...
        echo '<td style="vertical-align:middle;text-align:center"><a style="font-size:14px;font-family:typo;font-weight:bold" href="accountcopy.php?q=3&show='.($show-1).'">&nbsp;<<&nbsp;</a></td>';
        $i = 1;
        while($i<=$total){
            echo '<td style="vertical-align:middle;text-align:center"><a style="font-size:14px;font-family:typo;font-weight:bold" href="accountcopy.php?q=3&show='.$i.'">&nbsp;'.$i.'&nbsp;</a></td>';
            $i++;
